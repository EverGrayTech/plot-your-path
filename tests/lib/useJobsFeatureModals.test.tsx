...
import { useJobsFeatureModals } from "../../src/lib/useJobsFeatureModals";
...
describe("useJobsFeatureModals", () => {
  ...
  it("opens modals, loads data, and handles AI token/config actions", async () => {
    const { result } = renderHook(() => useJobsFeatureModals());
    ...
    await waitFor(() => expect(result.current.outcomeInsights?.confidence_message).toBe("Moderate"));

    await act(async () => {
      await result.current.handleUpdateAIConfig("job_parsing", { provider: "anthropic" });
      result.current.setTokenInputs((previous) => ({ ...previous, job_parsing: "secret" }));
    });
    ...
    await act(async () => {
      await result.current.handleUpdateToken("job_parsing");
      await result.current.handleClearToken("job_parsing");
      await result.current.handleHealthcheck("job_parsing");
    });

    expect(services.aiSettings.updateAISetting).toHaveBeenCalled();
    expect(services.aiSettings.updateAISettingToken).toHaveBeenCalledWith("job_parsing", "secret");
    expect(services.aiSettings.clearAISettingToken).toHaveBeenCalledWith("job_parsing");
    expect(result.current.healthByFamily.job_parsing).toMatch(/OK/i);
  });
