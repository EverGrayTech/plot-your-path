...
import { useRoleDetailState } from "../../src/lib/useRoleDetailState";
...
vi.mock("../../src/lib/rolesPageUtils", async () => {
  const actual = await vi.importActual<typeof import("../../src/lib/rolesPageUtils")>(
    "../../src/lib/rolesPageUtils",
  );
  return {
    ...actual,
...
describe("useRoleDetailState", () => {
  beforeEach(() => {
    services.roles.getRole.mockResolvedValue(role);
    services.roles.updateRoleStatus.mockResolvedValue(undefined);
    services.skills.getSkill.mockResolvedValue({
      id: 9,
      name: "TypeScript",
      category: null,
      usage_count: 1,
    ...
    services.workflows.updateNextAction.mockResolvedValue({
      next_action_at: "2026-03-22T12:00:00.000Z",
    });
    ...
    services.aiGeneration.syncResumeProfile.mockResolvedValue({
      ingested_count: 1,
      source_used: "profile",
      source_record_id: "id",
    });
    ...
    services.aiGeneration.analyzeRoleFit.mockResolvedValue({
      id: 2,
      role_id: 1,
      fit_score: 80,
      recommendation: "go",
    ...
    services.aiGeneration.scoreRoleDesirability.mockResolvedValue({
      id: 3,
      company_id: 1,
      role_id: 1,
      total_score: 7,
      ...
      score_scope: "company",
      fallback_used: true,
      cache_expires_at: "2026-03-19T00:00:00.000Z",
      is_stale: false,
      provider: "browser",
      model: "v1",
      version: "v1",
    ...
    services.aiGeneration.refreshDesirabilityScore.mockResolvedValue({
      id: 4,
      company_id: 1,
      role_id: 1,
      total_score: 8,
      ...
      score_scope: "company",
      fallback_used: true,
      cache_expires_at: "2026-03-20T00:00:00.000Z",
      is_stale: false,
      provider: "browser",
      model: "v1",
      version: "v1",
  ...
  it("loads role detail and exercises major handlers", async () => {
    const loadRoles = vi.fn().mockResolvedValue(undefined);
    const { result } = renderHook(() => useRoleDetailState({ loadRoles }));

    act(() => {
      result.current.openRoleDetail(1);
    });

    await waitFor(() => expect(result.current.selectedRole?.id).toBe(1));
    ...
    expect(services.roles.updateRoleStatus).toHaveBeenCalledWith(1, "submitted");
    expect(services.aiGeneration.analyzeRoleFit).toHaveBeenCalledWith(1);
    ...
    expect(services.aiGeneration.generateQuestionAnswers).toHaveBeenCalledWith(1, [
      "Why this role?",
    ]);
    expect(services.aiGeneration.generateInterviewPrepPack).toHaveBeenCalledWith(1);
    expect(services.aiGeneration.regenerateInterviewPrepSection).toHaveBeenCalledWith(
      1,
      "likely_questions",
    );
    ...
    expect(services.aiGeneration.scoreRoleDesirability).toHaveBeenCalledWith(1);
  ...
  it("loads skill detail and supports closing state", async () => {
    const { result } = renderHook(() => useRoleDetailState({ loadRoles: vi.fn() }));
    ...
    act(() => {
      ...
      result.current.closeRoleDetail();
    });
  ...
  it("covers guard clauses and error branches", async () => {
    services.roles.getRole.mockRejectedValueOnce(new Error("detail fail"));
    ...
    services.aiGeneration.scoreRoleDesirability.mockRejectedValueOnce(
      new Error("desirability failed"),
    );
    services.aiGeneration.syncResumeProfile.mockRejectedValueOnce(new Error("sync failed"));
    vi.stubGlobal("navigator", {
      clipboard: { writeText: vi.fn().mockRejectedValue(new Error("copy failed")) },
    });

    const emptyQuestionsHook = renderHook(() => useRoleDetailState({ loadRoles }));

    await act(async () => {
      emptyQuestionsHook.result.current.openRoleDetail(1);
    });

    await waitFor(() => expect(emptyQuestionsHook.result.current.selectedRole?.id).toBe(1));
    ...
    await waitFor(() =>
      expect(emptyQuestionsHook.result.current.selectedInterviewPrepId).toBe(999),
    );
    ...
    expect(String(emptyQuestionsHook.result.current.materialsError)).toMatch(
      /qa generation failed/i,
    );
    expect(String(emptyQuestionsHook.result.current.interviewPrepError)).toMatch(
      /Each interview prep section requires at least one/i,
    );
    expect(String(emptyQuestionsHook.result.current.resumeTuningError)).toMatch(
      /copy resume tuning suggestions|sync failed|resume tuning failed/i,
    );
    expect(String(emptyQuestionsHook.result.current.desirabilityError)).toMatch(
      /desirability failed/i,
    );
  });
});
