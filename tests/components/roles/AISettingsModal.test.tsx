...
import { AISettingsModal } from "../../../src/components/roles/AISettingsModal";

describe("AISettingsModal", () => {
  const setting = {
    operation_family: "role_parsing" as const,
    provider: "openai",
    model: "gpt-4o-mini",
  ...
  it("updates config, token input, and action buttons", () => {
    ...
    render(
      <AISettingsModal
        aiSettings={[setting]}
        error={null}
        healthByFamily={{ role_parsing: "OK" }}
        loading={false}
    ...
    expect(onUpdateConfig).toHaveBeenCalledWith("role_parsing", { provider: "anthropic" });
    expect(onUpdateConfig).toHaveBeenCalledWith("role_parsing", { model: "gpt-4.1" });
    expect(onUpdateConfig).toHaveBeenCalledWith("role_parsing", { token_label: "Updated token" });
    expect(onTokenInputChange).toHaveBeenCalledWith("role_parsing", "secret");
    expect(onUpdateToken).toHaveBeenCalledWith("role_parsing");
    expect(onClearToken).toHaveBeenCalledWith("role_parsing");
    expect(onHealthcheck).toHaveBeenCalledWith("role_parsing");
